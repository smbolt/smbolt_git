    ...
    public override async Threading.Task<TaskResult> ProcessTaskAsync(Func<bool> checkContinue)
    {
      ...
      try
      {
        taskResult = await Threading.Task.Run<TaskResult>(() =>
        {
          ...
          ExchangeServiceManager exServMgr = new ExchangeServiceManager(opsEmailAddress, TokenMaker.GenerateToken2(tokenizedPassword));
          tokenizedPassword = null;

          //List<MonitoredTask> monitoredTasks;
          ...
          if (IsDryRun)
            ...
            exServMgr.MoveItems(successItems, "Overdue Processed");
          ...
          if (errorItems.Count > 0)
          {
            ...
            exServMgr.MoveItems(errorItems, "Overdue Error");
