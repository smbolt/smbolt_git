...
{
  public class ModuleBase : MarshalBase, IModule
  {
    ...
    protected string ModuleConfigName {
      get;
      private set;
    }
    ...
    protected string ModuleNameAndVersionString {
      get {
        return Get_ModuleNameAndVersionString();
      }
    }
    protected string ModuleCopyrightString  {
      get {
        return Get_ModuleCopyrightString();
      }
    }
    protected bool IsMarshalled = false;
    ...
    public void Initialize(Assembly module)
    {
      if (g.AppInfo.RunningInNonDefaultAppDomain)
      {
        ...
        if (!g.aIsInitialized)
        {
          ...
          {
            throw new Exception("An exception occurred in ModuleBase while attempting to load the AppConfig file.", ex);
          }
        }
