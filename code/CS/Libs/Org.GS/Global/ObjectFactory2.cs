  ...
  public class ObjectFactory2 : IDisposable
  {
                    ...
                    if (propXMap.WrapperElement.IsNotBlank())
                    {
                      ...
                      if (xmlValue.Name.LocalName == propXMap.WrapperElement)
                      {
                        if (propXMap.CollectionElements.IsNotBlank())
                        {
                          ...
                          //currently only supporting native lists of native types contained as the content of a wrapped sequence of elements
                          if (argTypes.Length > 0)
                          {
                            switch (argTypes.Length)
                            {
    ...
    public XElement Serialize(object o, int serializationDepth)
    {
      try
      {
        ...
        string elementName = o.GetType().Name;
        if (xMap.Name.IsNotBlank())
          elementName = xMap.Name;
        ...
        // Get the object's XMap properties
        List<PropertyInfo> piList = GetXMapProperties(type);
        ...
        foreach (PropertyInfo pi in piList)
        {
          ...
          // Dont map "Parent" references to Xml
          if (propXMap != null && propXMap.MyParent)
            continue;

          string propertyName = GetPropertyName(pi.Name, propXMap.Name);
          object value = pi.GetValue(o, null);

          Type propertyObjectType = pi.PropertyType;
          XMap propertyTypeXMap = pi.PropertyType.GetCustomAttribute<XMap>();

          if (value != null)
          {
            XObjectType xObjectType = pi.GetXObjectType();
            ...
            switch (xObjectType)
            {
              ...
              case XObjectType.GenericCollectionBased:
                ...
                foreach (PropertyInfo collectionProperty in collectionProperties)
                {
                  ...
                  else
                  {
                    ...
                    //throw new Exception("Only XType=XType.Attribute types of properties are currently supported on objects " +
                    //                    "that derive from generic collections.  Object type is '" + pi.GetType().Name + "' " +
                    //                    "property name is '" + collPropertyName + "'.");
                  }
                }
                ...
                string wrapperElement = propertyTypeXMap == null ? String.Empty : propertyTypeXMap.WrapperElement;
              ...
              default:  // XObjectType = Simple
                ...
                {
                  if (propXMap.XType == XType.Element)
                    xml.Add(new XElement(propertyName, valueString));
                  else
                    xml.Add(new XAttribute(propertyName, valueString));
                }
                break;
            }
          }
    ...
    private Type AttemptToLocateType(string className)
    {
      ...
      {
        if (assembly.CustomAttributes.OfType<Org.GS.OrgAssemblyTag>() != null)
        {
