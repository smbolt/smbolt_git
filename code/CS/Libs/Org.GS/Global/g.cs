...
namespace Org.GS
{
  public static class g
  {
    ...
    public static bool ForceModuleConfigs = false;
    public static bool aIsInitialized = false;
    ...
    public static string crlf3 = g.crlf + g.crlf + g.crlf;
    ...
    public static string MsgTitle(string message)
    {
      return AppName + " - " + (message?.ObjectToTrimmedString());
    }
    ...
    public static string AppDataPath
    {
      get
      {
        ...
        _appDataPath = Get_AppDataPath();

        if (_appDataPath.IsBlank())
        {
    ...
    {
      StartupLogging.WriteStartupLog("g.Initialize is starting");

      try
      {
        Tags = new List<string>();

        string[] cmdArgs = Environment.CommandLine.Split(Constants.SpaceDelimiter, StringSplitOptions.RemoveEmptyEntries);

        g.Perf = new PerfManager();

        StartupLogging.WriteStartupLog("g.Initialize 1");

        int baseDirArg = -1;
        for (int i = 0; i < cmdArgs.Length; i++)
        {
          if (cmdArgs[i].ToLower() == "-basedir")
          {
            baseDirArg = i;
            continue;
          }

          if (baseDirArg > -1 && i == baseDirArg + 1)
          {
            g.AlternateAppConfigPath = cmdArgs[i];
            break;
          }
        }
        ...
        StartupLogging.WriteStartupLog("g.Initialize is complete");
    ...
    private static string Get_AppDataPath()
    {
      ...
      var appType = g.AppInfo.OrgApplicationType;
      if (g.AppInfo.ParentAppDomainAppType != ApplicationType.NotSet)
        appType = g.AppInfo.ParentAppDomainAppType;
    ...
    [DebuggerStepThrough]
    public static string GetVariableValue(string ciName)
    {
      if (AppConfig == null)
        return String.Empty;

      if (AppConfig.ContainsKey(ciName))
        return AppConfig.GetVariableValue(ciName);
    ...
    public static List<string> GetListFromDictionary(string dictName, DictionaryPart dictionaryPart)
    {
      return GetDictionary(dictName).ToList(dictionaryPart);
    }
    ...
    public static Type GetTypeValue(object o)
    {
      try
      {
        if (o != null)
      ...
      {
        throw new Exception("An exception occurred while attempting to translate the value " + (o == null ? "NULL" : o.ToString()) +
                            " to a System.Type value.", ex);
      }
    }
    ...
    //[DebuggerStepThrough]
    public static ConfigFtpSpec GetFtpSpec(string prefix)
    {
      string specName = g.EnvPrefix + prefix;
      ConfigFtpSpec spec = (ConfigFtpSpec)g.AppConfig.GetCO<ConfigFtpSpec>(specName);
      return spec;
    }
    ...
    [DebuggerStepThrough]
    public static ConfigWsSpec GetWsSpec(string prefix)
    {
      string specName = g.EnvPrefix + prefix;
      ConfigWsSpec spec = (ConfigWsSpec)g.AppConfig.GetCO<ConfigWsSpec>(specName);
      return spec;
    }
    ...
    {
      if (!Enum.IsDefined(typeof(TEnum), strEnumValue))
        throw new Exception("The value '" + strEnumValue + "' is not valid for enum type '" + typeof(TEnum).Name + "'.");
    ...
    private static RuntimeEnvironment Get_RuntimeEnvironment()
    {
      if (g.SystemInfo == null || g.SystemInfo.ComputerName.IsBlank())
        return RuntimeEnvironment.Unknown;

      string computerName = g.SystemInfo.ComputerName.Trim().ToLower();

      if (computerName.Contains("okc1web0"))
        return RuntimeEnvironment.Prod;

      if (computerName.Contains("okc1web1"))
        return RuntimeEnvironment.Test;

      return RuntimeEnvironment.Dev;
    }

    private static string Get_BranchName()
    {
      if (g.RuntimeEnvironment != RuntimeEnvironment.Dev)
        return String.Empty;

      string runtimePath = g.ExecutablePath;

      if (runtimePath.ToLower().StartsWith(@"c:\dev-main\"))
      {
        string branchPath = runtimePath.Substring(12);
        string branchName = branchPath.Split(Constants.SlashDelimiter, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault().DbToString();
        return branchName;
      }
    ...
    {
      return "[_" + DateTime.Now.ToString("yyyyMMdd-HHmmss") + "_]-";
    }
