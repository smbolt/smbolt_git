...
namespace Org.GS
{
  public class FileSystemItemSet : SortedList<string, FileSystemItem>
  {
    ...
    {
      return this.Values.Where(f => f.FileSystemItemType == FileSystemItemType.Folder).ToList();
    }

    private List<FileSystemItem> Get_Files()
    {
      return this.Values.Where(f => f.FileSystemItemType == FileSystemItemType.File).ToList();
    }
    ...
    {
      return this.Values.Where(f => f.FullPath.ToLower() == folderName.ToLower()).FirstOrDefault();
    }
