...
namespace Org.GS
{
  public class NetworkHelper
  {
    ...
    {
      IPAddress ipAddress = DnsResolveIPV4Address(hostName);
      return ipAddress.ToString();
    }
    ...
    public static IPAddress DnsResolveIPV4Address(string address)
    {
      IPAddress[] ipAddresses = Dns.GetHostEntry(address).AddressList;
      ...
      {
        if (ipAddress.ToString().IsIPV4Address())
        {
          return ipAddress;
        }
      }

      throw new ArgumentException("No IPV4 ip address exists for '" + address + "'.", "address");
    }
