...
namespace Org.GS
{
  public class FileSystemUtility : IDisposable
  {
    ...
    public string DeleteDirectoryRecursive(string path, int level = 0)
    {
      ...
      if (this.CreateReport)
      {
        ...
        sb.Append(g.crlf + fileInfo.ToFileActionReport(_isDryRun, "DELETE", level) + g.crlf);
      }
      ...
      foreach (string file in files)
      {
        if (_isDryRun)
        {
          if (this.CreateReport)
          {
            ...
            sb.Append(fileInfo.ToFileActionReport(_isDryRun, "DELETE", level) + g.crlf);
          }
        }
        else
        {
      ...
      {
        sb.Append(DeleteDirectoryRecursive(folder, level + 1) + g.crlf);
      }
