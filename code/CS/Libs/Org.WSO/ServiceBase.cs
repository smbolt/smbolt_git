...
namespace Org.WSO
{
  ...
  public class ServiceBase : IDisposable
  {
    ...
    {
      if (processorKey.StartsWith("ServiceEngineCommandProcessor"))
        return new Org.WSO.RequestProcessors.RequestProcessorFactory();

      return _serviceState.GetRequestProcessorFactoryFromAppDomain(assemblyName, processorKey, catalogName, catalogType, objectTypeName);
    }
    ...
    public TaskResult GetAppDomainReport()
    {
      if (_serviceState == null)
        return new TaskResult("GetAppDomainReport", "The _serviceState object in ServiceBase is null.", TaskResultStatus.Failed);

      return _serviceState.GetAppDomainReport();
    }

    public TaskResult FlushAppDomains()
    {
      if (_serviceState == null)
        return new TaskResult("FlushAppDomains", "The ServiceState property of the ServiceBase object is null.", TaskResultStatus.Failed);

      return _serviceState.FlushAppDomains();
    }
    ...
    {
      if (_serviceState == null)
        return ComponentLoadMode.NotSet;

      return _serviceState.ComponentLoadMode;
    }

    private string Get_CatalogStem()
    {
      if (_serviceState == null)
        return String.Empty;

      return _serviceState.CatalogStem;
    }

    private string Get_CatalogEnvironment()
    {
      if (_serviceState == null)
        return String.Empty;

      return _serviceState.CatalogEnvironment;
    }

    private string Get_CatalogTaskNode()
    {
      if (_serviceState == null)
        return String.Empty;

      return _serviceState.CatalogTaskNode;
    }

    private string Get_CatalogName()
    {
      if (_serviceState == null)
        return String.Empty;

      return _serviceState.CatalogName;
    }

    public CatalogEntry GetCatalogEntry(string transName)
    {
      if (_serviceState == null)
        throw new Exception("The ServiceState object is null.");

      return _serviceState.GetCatalogEntry(transName);
    }

    public string Get_WebServiceName()
    {
      if (_serviceState == null)
        return this.AppName;

      return _serviceState.WebServiceName;
    }
    ...
    public string Get_ComputerName()
    {
      ...
      return _serviceState.ComputerName;
    }
