...
namespace Org.Dx.Business.TextProcessing
{
  public class TextExtractor : IDisposable
  {
    ...
    public SortedList<int, string> ExtractPageTextFromPdf(bool allowDebugBreak, string fullPath, ExtractionMap extractionMap = null)
    {
      try
      {
        if (!File.Exists(fullPath))
        ...
        if (extractionMap != null)
        {
          ...
          if (extractionMap.ExtractSectionSet == null)
            throw new Exception("The ExtractSectionSet collection of the ExtractionMap is null.");
        }

        var pageTextList = new SortedList<int, string>();
        ...
        using (PdfReader reader = new PdfReader(fullPath))
        {
          for (int i = 1; i <= reader.NumberOfPages; i++)
          {
            ...
            {
              string pageText = PdfTextExtractor.GetTextFromPage(reader, i);
              pageText = pageText.RemoveExtraSpacesAndLines();
            ...
            else
            {
              ...
              string rawText = sb.ToString().RemoveExtraSpacesAndLines();
              pageTextList.Add(i, rawText);
            }
