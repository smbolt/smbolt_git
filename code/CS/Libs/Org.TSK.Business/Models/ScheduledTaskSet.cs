...
namespace Org.TSK.Business.Models
{
  public class ScheduledTaskSet : List<ScheduledTask>
  {
    ...
    public void LoadTasksToRun(int loadIntervalSeconds, bool scheduleOnceNow, bool viewOnly)
    {
      // Lock this object, wait for up to a second
      if (Monitor.TryEnter(TaskSet_LockObject, 1000))
      {
        try
        {
          ...
          LoadTasksToRun(now, now.AddSeconds(loadIntervalSeconds), scheduleOnceNow, viewOnly);
        }
        finally
        {
          Monitor.Exit(TaskSet_LockObject);
    ...
    public void LoadTasksToRun(DateTime intervalStart, DateTime intervalEnd, bool scheduleOnceNow, bool viewOnly)
    {
      ...
      foreach (var task in this)
      {
        ...
        ScheduledRunSet runSet = task.GetScheduledRunSet(intervalStart, intervalEnd, scheduleOnceNow, viewOnly);
    ...
    public string GetParametersReport()
    {
      ...
      foreach (var scheduledTask in this)
      {
        ...
        sb.Append(scheduledTask.TaskName + ": " +
                  ...
                  "--------------------------------------------------------------------------------------------------------" + g.crlf +
                  "Schedule Name: " + (scheduledTask.TaskSchedule != null ? scheduledTask.TaskSchedule.ScheduleName : "NO SCHEDULE DEFINED ") + g.crlf);
          ...
          switch (parm.ParameterType.Name)
          {
            ...
            case "List`1":
              ...
              else
              {
                ...
                {
                  sb.Append(g.crlf + String.Empty.PadTo(12) + item.PadTo(40));
                }
              }
              break;
