...
namespace Org.TSK.Business.Models
{
  public class ScheduledTask
  {
    ...
    public ScheduledRunSet GetScheduledRunSet(DateTime intervalStart, DateTime intervalEnd, bool viewOnly)
    {
      try
      {
        ...
        foreach (var e in this.TaskSchedule.TaskScheduleElements.Where(t => t.IsActive))
        {
        ...
        if (!viewOnly && this.RunUntilOverride && runSet.Count > 0)
        {
    ...
    private void Load(TaskConfig t)
    {
      ...
      foreach (var schedElement in t.TaskSchedule)
      {
        ...
        taskScheduleElement.IsManaged = false;
        taskScheduleElement.ElementNumber = taskScheduleElement.TaskScheduleElementId;
