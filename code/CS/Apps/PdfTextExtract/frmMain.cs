...
namespace Org.PdfTextExtract
{
  public partial class frmMain : frmToolWindowParent
  {
    ...
    private bool _fDisplayTextClosed;
    private bool _pdfDocumentLoaded;
    ...
    private void Action(object sender, EventArgs e)
    {
      ...
      switch (action)
      {
        ...
        case "Exit":
          ...
          //{
          //  var row = gvFiles.Rows[i];
          //  sb.Append(row.Cells[0].Value.ToString() + "|" +
          //            row.Cells[1].Value.ToString() + "|" +
          //            row.Cells[2].Value.ToString() + g.crlf); 
          //}
    ...
    private void LoadFiles()
    {
      ...
      try
      {
        ...
        searchParms.RootPath = cboRootFolder.Text;
    ...
    private void gvFiles_SelectionChanged(object sender, EventArgs e)
    {
      ...
      try
      {
        ...
        {
          _text = textExtractor.ExtractTextFromPdf(fullFilePath);
					if (g.CI("InDiagnosticsMode").ToBoolean())
						Org.GS.TextProcessing.Text.InDiagnosticsMode = true;
        }
        ...
        if (_twMgr.ToolPanels.ContainsKey("PdfViewer"))
				{
        ...
        if (_twMgr.ToolPanels.ContainsKey("TextExtractDesigner") && formatName != "UNDEFINED")
        {
    ...
    private OSFolder RecurseFileSystem(SearchParms searchParms, bool wireUpEvents)
    {
      ...
      rootFolder.FullPath = cboRootFolder.Text.Trim();
      rootFolder.RootFolderPath = cboRootFolder.Text.Trim();
      ...
      try
      {
        for (int i = 0; i < gvFiles.Rows.Count; i++)
        {
          ...
          using (var textExtractor = new TextExtractor())
          {
            ...
            var text = textExtractor.ExtractTextFromPdf(fullFilePath);
    ...
    private void LoadRecogSpecSet()
    {
      try
      {
        ...
        var recogSpecFiles = Directory.GetFiles(g.ImportsPath, "*.rcg").ToList();
        ...
        using (var f = new ObjectFactory2())
        {
          foreach (var recogSpecFile in recogSpecFiles)
          {
            ...
            var recogSpecSet = f.Deserialize(xml) as RecogSpecSet;
    ...
    private void LoadExtractSpecSet()
    {
      try
      {
        ...
        var extractSpecFiles = Directory.GetFiles(g.ImportsPath, "*.ext").ToList();
        ...
        using (var f = new ObjectFactory2())
        {
          foreach (var extractSpecFile in extractSpecFiles)
          {
            ...
            var extractSpecSet = f.Deserialize(xml) as ExtractSpecSet;
    ...
    {
      var folderList = g.GetList("FileNameFilters");
      ...
      foreach (var folder in folderList)
      {
        cboFileNameFilters.Items.Add(folder);
      }
