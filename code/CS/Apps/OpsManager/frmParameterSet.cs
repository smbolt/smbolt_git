...
namespace Org.OpsManager
{
  public partial class frmParameterSet : Form
  {
    ...
    {
      DialogResult result = MessageBox.Show("Save these changes to the ScheduledTaskParameters table in the Task Scheduling database?",
      ...
      if (result == DialogResult.Yes)
      {
        ...
        if (gvParametersInSet.Rows.Count > 0)
        {
          ...
          {
            TaskParameter parm = new TaskParameter();
            if (row.Cells[0].Value.ToInt32() != 0)
            {
              parm.ParameterID = row.Cells[0].Value.ToInt32();
              parm.ModifiedBy = Environment.UserName;
              parm.ModifiedDate = currentTime;
            }
            else
            {
              parm.CreatedBy = Environment.UserName;
