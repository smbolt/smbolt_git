    ...
    private void SaveToDb()
    {
      if (ValidUI())
      {
        ...
        if (dtpStartDate.Checked)
          _opsData.CurrentScheduleElement.StartDate = dtpStartDate.Value.Date;

        if (dtpStartTime.Checked)
          _opsData.CurrentScheduleElement.StartTime = dtpStartTime.Value.TimeOfDay;

        if (dtpEndDate.Checked)
          _opsData.CurrentScheduleElement.EndDate = dtpEndDate.Value.Date;

        if (dtpEndTime.Checked)
          _opsData.CurrentScheduleElement.EndTime = dtpEndTime.Value.TimeOfDay;
