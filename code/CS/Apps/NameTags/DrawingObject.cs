...
namespace NameTags
{
	...
	public class DrawingObject
	{
		...
		private Enums.ObjectType _type;
		...
		private System.Windows.Forms.HorizontalAlignment _textHorzAlign;
		public System.Windows.Forms.HorizontalAlignment TextHorizAlign
		{
			get { return _textHorzAlign; }
			set 
            { 
		...
		public DrawingObject()
		{
			...
			_textHorzAlign = System.Windows.Forms.HorizontalAlignment.Left;
            _isLocked = false;
		}

			
		public void DrawObject(Graphics gr, float scale, PointF origin, PointF adjust, Enums.DisplayMode mode, Dictionary<string,string> dictionary)
		{
            ...
            try
            {
                ...
                switch (this._type)
                {
                    case Enums.ObjectType.TextObject:
                        ...
                        if (mode != Enums.DisplayMode.Designer)
                        {
                    ...
                    case Enums.ObjectType.DiplomaPicture:
                        Bitmap pic = new Bitmap(250, 250);
                        try
                        {
                            if (mode == Enums.DisplayMode.Designer)
                            {
                                pic = (Bitmap)Bitmap.FromFile(ConfigHelper.DefaultPhotoPath + ConfigHelper.DefaultPhotoSampleFileName);
                            }
                            else
                            {
                                if (!File.Exists(ConfigHelper.DefaultPhotoPath + dictionary["@fullname"].ToString() + ".jpg"))
                                {
                                    pic = (Bitmap)Bitmap.FromFile(ConfigHelper.DefaultPhotoPath + ConfigHelper.DefaultPhotoSampleFileName);
                                }
                                else
                                {
                                    pic = (Bitmap)Bitmap.FromFile(ConfigHelper.DefaultPhotoPath + dictionary["@fullname"].ToString() + ".jpg");
                                }
                            }
                        }
                        catch (FileNotFoundException fe)
                        {
                            MessageBox.Show("The following file is missing:\r\n" + fe.Message, "NameTagCreator - File not found");
                        }
                        catch (Exception e)
                        {
                            MessageBox.Show(e.Message, "NameTagCreator - Error");
                        }
                ...
                if (_selected & mode == Enums.DisplayMode.Designer)
                {
                    if (_isLocked)
                    {
    ...
    public void SetFontSize(float fontSize)
    {
      ...
      this.TextFont = new Font(this.TextFont.FontFamily, fontSize);
    }
